...
def main():
    ...
    # ============================================================
    # SSA: Inject custom attention layer spec
    # This replaces the default attention with SSADotProductAttention
    # which uses a modified softmax: softmax(n * b * ln(1 + x))
    # ============================================================
    ssa_layer_spec = get_ssa_gpt_layer_spec(
        num_experts=None,  # Dense model (no MoE)
        moe_grouped_gemm=False,
        qk_layernorm=False,
        ssa_n=1.5,  # SSA parameter n (trainable, init at 1.5)
        ssa_b=0.8,  # SSA parameter b (fixed at 0.8)
    )
    recipe.model.config.transformer_layer_spec = ssa_layer_spec
    # Disable fused softmax since we use custom implementation
    recipe.model.config.masked_softmax_fusion = False
    logger.info("SSA: Using SSADotProductAttention with modified softmax")
