"""
Generate text from Baby Luciole SSA Triton checkpoints.

This script loads a Baby Luciole model configured with the Triton SSA
...
DEFAULT_CHECKPOINT = (
    "/tmpdir/m24047brmn/nemo_1b/output/baby_luciole-ssa-triton/"
    "checkpoints/baby_luciole-ssa-triton-step=0019999-last"
)
DEFAULT_PROMPTS = [
...
def load_model(
    ...
    logger.info(
        "Triton SSA config enabled (compiled_bda=%s, force_contiguous_qkv=%s)",
        compiled_bda,
...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Generate text from Baby Luciole SSA Triton checkpoint"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        default=False,
        help="Enable/disable torch.compile'd BDA path in Triton layer spec.",
    )
    parser.add_argument(
