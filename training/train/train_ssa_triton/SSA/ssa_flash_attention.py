# Copyright (c) 2025, SSA Flash Attention Triton - autograd.Function wrapper
# Wraps the Triton kernels (tutorial-based structure) into a differentiable
...
class SSAFlashAttnFunc(Function):
    """
    Differentiable SSA Flash Attention (Triton — tutorial-based kernel).
    ...
    def forward(ctx, q, k, v, softmax_scale, ssa_n, ssa_b, causal, dropout_p, training):
        ...
        out, m = ssa_flash_attn_forward(q, k, v, softmax_scale, ssa_n_f32, ssa_b_f32, causal)
...
def ssa_flash_attention(
    ...
    """
    High-level API for SSA Flash Attention Triton (tutorial-based kernel) with native GQA.
