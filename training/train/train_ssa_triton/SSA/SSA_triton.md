# SSA Triton Implementation Notes

This document records the full set of changes made to build and stabilize the `triton` SSA Triton path based on the Triton fused-attention tutorial structure (`06-fused-attention`), plus integration and validation changes around it.
...
- `test/check_ssa_triton_parity.py`
- `test/check_ssa_triton_parity.slurm`

## 2) Kernel architecture changes (`ssa_triton_kernel.py`)
...
`ssa_flash_attn_forward` and `ssa_flash_attn_backward` enforce:
...
## 3) Autograd wrapper updates (`ssa_flash_attention.py`)

Updated naming/state semantics to reflect stored tutorial normalizer:
- save `M` buffer from forward (not row-sum `L`).
- backward consumes `M`.

No public API change for model callsites (`ssa_flash_attention(...)` unchanged).

## 4) Model integration (`ssa_triton_attention.py`, `ssa_triton_layer_specs.py`)

`SSATritonAttention` uses:
- Megatron format conversion `[S,B,H,D] <-> [B,H,S,D]`
- `ssa_flash_attention(...)` call
- output dropout after fused attention output (same module behavior retained)

Layer spec wiring in `ssa_triton_layer_specs.py`:
- `SSATritonAttention` used as core attention.
...
### 5.1 triton-only launcher policy

Training launcher is pinned to `triton`:
- rejects non-`triton` `SSA_KERNEL_VERSION`.
...
- `SSA_N=1.5`
- `SSA_B=0.8`
- `SSA_KERNEL_VERSION=triton`

## 6) Parity tooling updates (`check_ssa_triton_parity.py`)

### 6.1 Internal-normalizer comparison handling

Because `triton` internal stored normalizer semantics differ from prior kernel/reference internals:
...
Script covers:
- kernel-level triton vs v3 (+ optional reference)
- determinism suite
...
It does not change SSA Triton kernel math.
