#!/usr/bin/env python3
"""Comprehensive parity checks for SSA Triton v4 kernels and integration paths."""
...
from SSA.ssa_flash_attention import USE_OPTIMIZED_KERNEL, ssa_flash_attention  # noqa: E402
from SSA.ssa_flash_attention_v4 import ssa_flash_attention_v4  # noqa: E402
from SSA.ssa_triton_attention import SSATritonAttention  # noqa: E402
from SSA.ssa_triton_v4_attention import SSATritonV4Attention  # noqa: E402
from SSA.ssa_triton_v4_kernel import (  # noqa: E402
    ssa_flash_attn_v4_backward,
    ssa_flash_attn_v4_forward,
)

if USE_OPTIMIZED_KERNEL:
    try:
        from SSA.ssa_triton_kernel_optimized import (  # noqa: E402
...
def _run_kernel_case(
    *,
    ...
    out4, l4 = ssa_flash_attn_v4_forward(q, k, v, scale, ssa_n, ssa_b, causal=causal)
    dout = torch.randn_like(out4)
    dq4, dk4, dv4, dn4, db4 = ssa_flash_attn_v4_backward(
        q, k, v, out4, dout, l4, scale, ssa_n, ssa_b, causal=causal
    )
    _check_finite("v4.out", out4)
    _check_finite("v4.l", l4)
    _check_finite("v4.dq", dq4)
    _check_finite("v4.dk", dk4)
    _check_finite("v4.dv", dv4)
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        if not _is_power_of_2(d_head):
            res = {
                ...
                "passed": False,
                "error": "Head dimension must be power-of-two for current v4 kernel.",
            }
            results.append(res)
        ...
        if n_ctx % 128 != 0:
            res = {
                ...
                "passed": False,
                "error": "N must be divisible by 128 for current v4 backward kernel.",
            }
            results.append(res)
...
def _run_determinism_case(
    ...
    out1, l1 = ssa_flash_attn_v4_forward(q, k, v, scale, n, b, causal=causal)
    dout = torch.randn_like(out1)
    dq1, dk1, dv1, dn1, db1 = ssa_flash_attn_v4_backward(
        q, k, v, out1, dout, l1, scale, n, b, causal=causal
    )

    out2, l2 = ssa_flash_attn_v4_forward(q, k, v, scale, n, b, causal=causal)
    dq2, dk2, dv2, dn2, db2 = ssa_flash_attn_v4_backward(
...
def _run_determinism_suite(args: argparse.Namespace, device: torch.device) -> dict[str, Any]:
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        if case["n_ctx"] % 128 != 0:
            out.append({"case": case, "passed": False, "error": "N must be divisible by 128 for v4 backward."})
            if args.fail_fast:
                break
            continue
        try:
...
def _run_module_suite(args: argparse.Namespace, device: torch.device) -> dict[str, Any]:
    ...
    if args.module_seq_length % 128 != 0:
        raise ValueError("module_seq_length must be divisible by 128 for v4 backward kernel.")
    ...
    try:
        ...
        for idx, layer in enumerate(layers, start=1):
            ...
            cfg_v4 = copy.deepcopy(cfg)
            m3 = SSATritonAttention(
                config=cfg_v3,
                layer_number=layer,
            ...
            v4 = v_ref.detach().clone().requires_grad_(True)

            out3 = m3(q3, k3, v3, attention_mask=None)
            out4 = m4(q4, k4, v4, attention_mask=None)
...
def _engine_forward(
    ...
    if engine == "v4":
        return ssa_flash_attention_v4(qd, kd, vd, scale, n, b, causal=causal, dropout_p=0.0, training=True)
...
def _run_training_drift_suite(args: argparse.Namespace, device: torch.device) -> dict[str, Any]:
    ...
    if n_ctx % 128 != 0:
        raise ValueError("train_seq_length must be divisible by 128 for v4 backward kernel.")
    if not _is_power_of_2(d_head):
        raise ValueError("train_head_dim must be power-of-two for current v4 kernel.")
    ...
    if bool(args.train_learnable_b):
        ...
        params_v4["b"] = torch.nn.Parameter(params_v4["b"].detach().clone())
    ...
    v4_optim_params = [params_v4["q"], params_v4["k"], params_v4["v"], params_v4["n"]]
    if bool(args.train_learnable_b):
        ref_optim_params.append(params_ref["b"])
        v3_optim_params.append(params_v3["b"])
        v4_optim_params.append(params_v4["b"])
    ...
    opt_v4 = torch.optim.AdamW(v4_optim_params, lr=args.train_lr, betas=(0.9, 0.99), weight_decay=0.0)

    worst_loss_v4_ref = 0.0
    worst_loss_v4_v3 = 0.0
    worst_param_v4_ref = 0.0
    worst_param_v4_v3 = 0.0
    history: list[dict[str, float]] = []

    for step in range(1, args.train_steps + 1):
        for opt in (opt_ref, opt_v3, opt_v4):
        ...
        loss_v4 = F.mse_loss(out_v4.float(), target)
        for name, loss in (("reference", loss_ref), ("v3", loss_v3), ("v4", loss_v4)):
        ...
        loss_diff_v4_ref = abs(float(loss_v4.item()) - float(loss_ref.item()))
        loss_diff_v4_v3 = abs(float(loss_v4.item()) - float(loss_v3.item()))

        param_diff_v4_ref = 0.0
        param_diff_v4_v3 = 0.0
        compare_names = ("q", "k", "v", "n", "b") if bool(args.train_learnable_b) else ("q", "k", "v", "n")
        for name in compare_names:
            param_diff_v4_ref = max(
                param_diff_v4_ref,
                float((params_v4[name].detach() - params_ref[name].detach()).abs().max().item()),
            )
            param_diff_v4_v3 = max(
                param_diff_v4_v3,
                float((params_v4[name].detach() - params_v3[name].detach()).abs().max().item()),
            )

        worst_loss_v4_ref = max(worst_loss_v4_ref, loss_diff_v4_ref)
        worst_loss_v4_v3 = max(worst_loss_v4_v3, loss_diff_v4_v3)
        worst_param_v4_ref = max(worst_param_v4_ref, param_diff_v4_ref)
        worst_param_v4_v3 = max(worst_param_v4_v3, param_diff_v4_v3)
        history.append(
            {
                "step": float(step),
                "loss_ref": float(loss_ref.item()),
                "loss_v3": float(loss_v3.item()),
                "loss_v4": float(loss_v4.item()),
                "loss_diff_v4_ref": loss_diff_v4_ref,
                "loss_diff_v4_v3": loss_diff_v4_v3,
                "param_diff_v4_ref": param_diff_v4_ref,
                "param_diff_v4_v3": param_diff_v4_v3,
            }
        )
        if step % max(1, args.train_steps // 10) == 0 or step == 1:
            print(
                f"[Train {step:04d}/{args.train_steps}] "
                f"loss_diff(v4,ref)={loss_diff_v4_ref:.3e} "
                f"loss_diff(v4,v3)={loss_diff_v4_v3:.3e} "
                f"param_diff(v4,ref)={param_diff_v4_ref:.3e} "
                f"param_diff(v4,v3)={param_diff_v4_v3:.3e}"
            )

    passed = (
        worst_loss_v4_ref <= args.train_loss_tol_v4_ref
        and worst_loss_v4_v3 <= args.train_loss_tol_v4_v3
        and worst_param_v4_ref <= args.train_param_tol_v4_ref
        and worst_param_v4_v3 <= args.train_param_tol_v4_v3
    )
    return {
        "name": "training_drift_suite",
        "passed": passed,
        "worst": {
            "loss_diff_v4_ref": worst_loss_v4_ref,
            "loss_diff_v4_v3": worst_loss_v4_v3,
            "param_diff_v4_ref": worst_param_v4_ref,
            "param_diff_v4_v3": worst_param_v4_v3,
        },
        "thresholds": {
            "train_loss_tol_v4_ref": args.train_loss_tol_v4_ref,
            "train_loss_tol_v4_v3": args.train_loss_tol_v4_v3,
            "train_param_tol_v4_ref": args.train_param_tol_v4_ref,
            "train_param_tol_v4_v3": args.train_param_tol_v4_v3,
        },
        "history": history,
    }
...
def _summarize_suite(suite: dict[str, Any]) -> None:
    ...
    if name == "training_drift_suite":
        ...
        print(
            "worst drift: "
            f"loss(v4,ref)={worst['loss_diff_v4_ref']:.3e}, "
            f"loss(v4,v3)={worst['loss_diff_v4_v3']:.3e}, "
            f"param(v4,ref)={worst['param_diff_v4_ref']:.3e}, "
            f"param(v4,v3)={worst['param_diff_v4_v3']:.3e}"
        )
        return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Comprehensive SSA Triton v4 parity checks")
    parser.add_argument("--arch", default="baby_luciole", type=str)
    parser.add_argument("--output_dir", default="/tmp/ssa_v4_parity", type=str)
    parser.add_argument("--name", default="ssa-v4-parity", type=str)
    ...
    parser.add_argument("--train_loss_tol_v4_ref", default=6e-3, type=float)
    parser.add_argument("--train_loss_tol_v4_v3", default=2e-3, type=float)
    parser.add_argument("--train_param_tol_v4_ref", default=3e-2, type=float)
    parser.add_argument("--train_param_tol_v4_v3", default=1e-2, type=float)
...
def main() -> None:
    ...
    print("=== SSA Triton v4 Parity Check ===")
