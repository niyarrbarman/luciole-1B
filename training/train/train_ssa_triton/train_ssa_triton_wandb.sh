...
NAME=${NAME:-"baby_luciole-ssa-triton-wandb-test-${SLURM_JOB_ID:-nojob}-${RUN_STAMP}"}
SEED=${SEED:-1234}
...
if [[ "${WANDB_MODE}" == "online" && -z "${WANDB_API_KEY}" ]]; then
    ...
    echo "Use: WANDB_API_KEY='...' sbatch train_ssa_triton_wandb.sh"
    echo "or:  api_key='...' sbatch train_ssa_triton_wandb.sh"
    exit 2
fi
