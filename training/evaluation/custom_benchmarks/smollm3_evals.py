...
from lighteval.metrics.normalizations import LogProbCharNorm, LogProbTokenNorm, harness_triviaqa_normalizer
from lighteval.metrics.utils.metric_utils import (SampleLevelMetricGrouping, SampleLevelMetric)
...
winogrande_tasks = [
    LightevalTaskConfig(
        ...
        # hf_revision="fd7b0145687ca3c563ab72e29a7604537d4af4d0",
        hf_avail_splits=["train", "validation"],
        evaluation_splits=["validation"],
        few_shots_split="train",
        metrics=qa_metrics,
...
triviqa_tasks = [
    LightevalTaskConfig(
        ...
        generation_size=256,
        stop_sequence=("\n",),
        metrics=[Metrics.exact_match(sample_params={"normalize_gold": harness_triviaqa_normalizer, "normalize_pred": harness_triviaqa_normalizer})],
...
mixeval_multichoice_easy = LightevalTaskConfig(
    ...
    hf_repo="MixEval/MixEval",
    hf_subset="MixEval",
    metrics=[llm_judge_mixeval_multichoice_flow_judge],
...
#     task.metrics = [
#         metrics
#         for metrics in task.metrics
#         if metrics.category != SamplingMethod.LOGPROBS
#     ]
