...
# Defaults
DATAMIX=${DATAMIX:-"/lustre/work/pdl17996/udl62d273/luciole-1B/training/train/train_ssa_triton_blackwell/luciole_phase2.json"}
OUTPUT_DIR=${OUTPUT_DIR:-"/lustre/work/pdl17996/udl62d273/phase2_output_blackwell/outputs"}
mkdir -p "$OUTPUT_DIR"
BATCH_SIZE=${BATCH_SIZE:-1024}
MICRO_BATCH_SIZE=${MICRO_BATCH_SIZE:-8}
# Blackwell backward tuning (safe subset: warp count + pipeline depth only). Sweep via env.
SSA_BWD_NUM_WARPS=${SSA_BWD_NUM_WARPS:-4}
SSA_BWD_NUM_STAGES=${SSA_BWD_NUM_STAGES:-2}
NAME=${NAME:-"nemotron-1B-SSA-Triton-phase2-blackwell-bs${BATCH_SIZE}-mbs${MICRO_BATCH_SIZE}-w${SSA_BWD_NUM_WARPS}s${SSA_BWD_NUM_STAGES}"}
SEED=${SEED:-1234}
...
echo "BWD warps:   $SSA_BWD_NUM_WARPS"
echo "BWD stages:  $SSA_BWD_NUM_STAGES"
...
srun apptainer exec \
  ...
  --env "SSA_BWD_NUM_WARPS=${SSA_BWD_NUM_WARPS}" \
  --env "SSA_BWD_NUM_STAGES=${SSA_BWD_NUM_STAGES}" \
  ...
  /lustre/work/pdl17996/udl62d273/luciole-1B/training/train/train_ssa_triton_blackwell/launcher.py \
  /lustre/work/pdl17996/udl62d273/luciole-1B/training/train/train_ssa_triton_blackwell/train_ssa_triton.py \
  --datamix "$DATAMIX" \
