...
def _inv_softplus(x: float) -> float:
    ...
    Used for initializing SSA parameters."""
    if x > 20:
        return x
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        ##SSA
        # Custom softmax parameters
        # SSA parameters - hardcoded initialization
        n_raw_init = 1.5
        b_raw_init = 0.8   
    ...
    ##SSA
    def get_ssa_params(self):
        """Get current n and b values with constraints applied."""
        # n = 1.0 + nn.functional.softplus(self.ssa_n_raw)  # n >= 1
        # b = nn.functional.softplus(self.ssa_b_raw)         # b > 0
        # return n, b
        n = self.ssa_n_raw  # CHANGED: Remove the constraint (no more 1.0 + softplus)
        b = 0.8  # CHANGED: Fixed to 0.8
        return n, b
    ...
    def ssa_transform(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #x = torch.clamp(x, min=-1000000, max=1000000)  # Add this!
        
        abs_x = torch.abs(x)
        sign_x = torch.sign(x)
        
        # Prevent overflow in log1p
        #log_term = torch.log1p(torch.clamp(b * abs_x, max=1e10))
        log_term = torch.log1p(b * abs_x) 
    ...
    def forward(
        ...
        # SSA transformation of attention scores
        #attention_scores = self.ssa_transform(attention_scores)
        
        # Normalize the attention scores to probabilities.
        #attention_probs = nn.functional.softmax(attention_scores, dim=-1)
        attention_probs = nn.functional.softmax(self.ssa_transform(attention_scores), dim=-1)
